...
const ContactSubmitted = ({ name, surname, email, message, onReset }: ContactSubmittedProps) => {
  return (
    <div className="bg-surface/90 border border-border/80 rounded-2xl p-6 sm:p-8 shadow-xl text-center flex flex-col items-center backdrop-blur-md">
      ...
      <p className="mt-3 font-body text-text-muted max-w-md text-sm leading-relaxed">
        Gracias por escribirnos, <strong className="text-text">{name} {surname}</strong>. Hemos recibido tu consulta y te responderemos en un plazo máximo de <strong className="text-primary">24 horas hábiles</strong> al correo <strong className="text-text">{email}</strong>.
      </p>
