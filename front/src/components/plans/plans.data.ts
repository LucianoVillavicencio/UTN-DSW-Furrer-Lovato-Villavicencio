...
export function enrichBackendPlan(plan: Plan): MembershipPlan {
  ...
  const isPremium =
    normName.includes('premium') ||
    (!normName.includes('básico') &&
      !normName.includes('basico') &&
  ...
  // Admins can edit each plan's features now and they are stored on the
  // backend. These name-guessed sets remain only as a fallback for a plan that
  // somehow still has none; the backend backfills automatically, so this should
  ...
  return {
    ...
    description:
      plan.description ||
      (isBasic
        ? 'Perfecto para empezar tu camino fitness'
        : isElite
          ? 'Experiencia fitness definitiva con beneficios premium'
          : 'Nuestro plan más popular con todo lo necesario'),
