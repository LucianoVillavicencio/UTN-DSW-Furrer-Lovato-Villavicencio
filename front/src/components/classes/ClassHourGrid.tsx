...
interface ClassHourGridProps {
  ...
  onSelectHour: (turno: TurnoClase) => void;
    ...
    <div className="mt-6">
      <label className="flex items-center justify-between text-xs font-bold uppercase tracking-wider text-text-muted mb-3">
        ...
        <span className="text-primary font-semibold text-[11px] bg-primary/10 px-2.5 py-0.5 rounded-full">
          {turnosForActiveExpandedDay.length} turnos
        </span>
      </label>

      {turnosForActiveExpandedDay.length === 0 ? (
        <div className="rounded-2xl border border-border/80 bg-background/50 p-8 text-center text-xs text-text-muted">
          No hay turnos programados para el día seleccionado. Elige otro día arriba.
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
          {turnosForActiveExpandedDay.map((t) => {
            const startDate = new Date(t.fechaHora);
            const endDate = new Date(startDate.getTime() + 3600000); // 1 hr class

            const startStr = startDate.toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" });
            const endStr = endDate.toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" });

            const cupoMax = t.cupoMaximo || 20;
            const cupoDispon = t.cupoDisponible ?? cupoMax;
            const inscritosCount = Math.max(0, cupoMax - cupoDispon);

            const isSelected = selectedHourTurno?.id === t.id;
            const isEnrolled = isEnrolledInTurno(t.id);

            // Dynamic badge color logic
            let badgeClass = "bg-emerald-500/10 text-emerald-400 border-emerald-500/20";
            let statusText = `${cupoDispon} libres`;
            if (isEnrolled) {
              badgeClass = "bg-primary/15 text-primary border-primary/30";
              statusText = "Inscripto";
            } else if (cupoDispon === 0) {
              badgeClass = "bg-red-500/10 text-red-400 border-red-500/20";
              statusText = "Lleno";
            } else if (cupoDispon <= 3) {
              badgeClass = "bg-amber-500/10 text-amber-400 border-amber-500/20";
              statusText = `¡Últimos ${cupoDispon}!`;
            }
              ...
              >
                ...
                <div className="mt-3 space-y-1 border-t border-border/40 pt-2 text-[10px]">
                  <div className="flex items-center justify-between text-text-muted">
                    ...
                    <strong className="text-text font-bold">{inscritosCount}</strong>
                  </div>

                  <div className="flex items-center justify-between pt-0.5">
                    <span className={`inline-block rounded-full border px-2 py-0.5 text-[9px] font-bold ${badgeClass}`}>
                      {statusText}
                    </span>
                  </div>
                </div>
